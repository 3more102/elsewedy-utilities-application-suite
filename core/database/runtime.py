...
from core.configuration import DB_PATH, DATABASE_URL, DB_BACKEND, SCHEMA_VERSION
...
def _ensure_schema_columns(conn):
    ...
    batch_cols=_table_columns(conn,'telemetry_ingest_batches')
    if batch_cols:
        if 'cbm_events_opened' not in batch_cols: conn.execute('ALTER TABLE telemetry_ingest_batches ADD COLUMN cbm_events_opened INTEGER NOT NULL DEFAULT 0')
        if 'cbm_events_resolved' not in batch_cols: conn.execute('ALTER TABLE telemetry_ingest_batches ADD COLUMN cbm_events_resolved INTEGER NOT NULL DEFAULT 0')
        if 'cbm_work_orders_created' not in batch_cols: conn.execute('ALTER TABLE telemetry_ingest_batches ADD COLUMN cbm_work_orders_created INTEGER NOT NULL DEFAULT 0')
    permission_cols=_table_columns(conn,'permissions')
    if permission_cols:
    ...
    work_cols=_table_columns(conn,'work_orders')
...
def init_db(hash_password):
    with db() as conn:
        ...
        CREATE TABLE IF NOT EXISTS permissions(
          id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT UNIQUE NOT NULL, name TEXT NOT NULL,
          category TEXT NOT NULL DEFAULT 'General', risk_level TEXT NOT NULL DEFAULT 'Standard', description TEXT DEFAULT ''
        );
        ...
        CREATE TABLE IF NOT EXISTS user_permission_overrides(
          ...
          permission_id INTEGER NOT NULL REFERENCES permissions(id) ON DELETE CASCADE,
          effect TEXT NOT NULL, reason TEXT NOT NULL DEFAULT '', expires_at TEXT,
          updated_by INTEGER REFERENCES users(id), updated_at TEXT NOT NULL,
          PRIMARY KEY(user_id,permission_id)
        );
        CREATE INDEX IF NOT EXISTS idx_user_permission_overrides_expiry ON user_permission_overrides(expires_at,effect);
        ...
        CREATE TABLE IF NOT EXISTS failure_modes(
          id INTEGER PRIMARY KEY AUTOINCREMENT, mode_no TEXT UNIQUE NOT NULL, parent_id INTEGER REFERENCES failure_modes(id) ON DELETE SET NULL,
          name TEXT NOT NULL, category TEXT NOT NULL DEFAULT 'General', description TEXT DEFAULT '', active INTEGER NOT NULL DEFAULT 1,
        ...
        CREATE INDEX IF NOT EXISTS idx_failure_modes_parent ON failure_modes(parent_id,active);
        CREATE INDEX IF NOT EXISTS idx_failure_modes_category ON failure_modes(category,active);
        CREATE TABLE IF NOT EXISTS asset_fmea(
          id INTEGER PRIMARY KEY AUTOINCREMENT, fmea_no TEXT UNIQUE NOT NULL, asset_id INTEGER NOT NULL REFERENCES assets(id) ON DELETE CASCADE,
          failure_mode_id INTEGER NOT NULL REFERENCES failure_modes(id), function_description TEXT DEFAULT '', failure_effect TEXT NOT NULL, failure_cause TEXT NOT NULL,
          current_controls TEXT DEFAULT '', recommended_action TEXT DEFAULT '', severity INTEGER NOT NULL, occurrence INTEGER NOT NULL, detectability INTEGER NOT NULL,
          rpn INTEGER NOT NULL, risk_band TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'Active', owner_id INTEGER REFERENCES users(id), review_due_date TEXT,
          last_reviewed_at TEXT, last_reviewed_by INTEGER REFERENCES users(id), created_by INTEGER REFERENCES users(id), created_at TEXT NOT NULL,
          updated_by INTEGER REFERENCES users(id), updated_at TEXT NOT NULL, UNIQUE(asset_id,failure_mode_id)
        );
        CREATE INDEX IF NOT EXISTS idx_asset_fmea_asset ON asset_fmea(asset_id,status,rpn);
        CREATE INDEX IF NOT EXISTS idx_asset_fmea_risk ON asset_fmea(risk_band,status,rpn);
        CREATE TABLE IF NOT EXISTS fmea_reviews(
          id INTEGER PRIMARY KEY AUTOINCREMENT, review_no TEXT UNIQUE NOT NULL, asset_fmea_id INTEGER NOT NULL REFERENCES asset_fmea(id) ON DELETE CASCADE,
          old_severity INTEGER NOT NULL, old_occurrence INTEGER NOT NULL, old_detectability INTEGER NOT NULL, old_rpn INTEGER NOT NULL,
          new_severity INTEGER NOT NULL, new_occurrence INTEGER NOT NULL, new_detectability INTEGER NOT NULL, new_rpn INTEGER NOT NULL,
          notes TEXT NOT NULL, reviewed_by INTEGER NOT NULL REFERENCES users(id), reviewed_at TEXT NOT NULL
        );
        CREATE INDEX IF NOT EXISTS idx_fmea_reviews_record ON fmea_reviews(asset_fmea_id,reviewed_at);
        CREATE TABLE IF NOT EXISTS rcm_strategies(
          id INTEGER PRIMARY KEY AUTOINCREMENT, strategy_no TEXT UNIQUE NOT NULL, asset_fmea_id INTEGER NOT NULL REFERENCES asset_fmea(id) ON DELETE CASCADE,
          functional_failure TEXT NOT NULL, consequence_classification TEXT NOT NULL, strategy_type TEXT NOT NULL, task_description TEXT NOT NULL,
          justification TEXT NOT NULL, interval_days INTEGER, linked_cbm_rule_id INTEGER,
          linked_pm_plan_id INTEGER, status TEXT NOT NULL DEFAULT 'Draft', owner_id INTEGER REFERENCES users(id),
          review_due_date TEXT, approved_by INTEGER REFERENCES users(id), approved_at TEXT, activated_by INTEGER REFERENCES users(id), activated_at TEXT,
          retired_by INTEGER REFERENCES users(id), retired_at TEXT, last_decision_comments TEXT DEFAULT '', created_by INTEGER REFERENCES users(id), created_at TEXT NOT NULL,
          updated_by INTEGER REFERENCES users(id), updated_at TEXT NOT NULL, UNIQUE(asset_fmea_id)
        );
        CREATE INDEX IF NOT EXISTS idx_rcm_strategies_status ON rcm_strategies(status,review_due_date);
        CREATE INDEX IF NOT EXISTS idx_rcm_strategies_fmea ON rcm_strategies(asset_fmea_id,status);
        CREATE INDEX IF NOT EXISTS idx_rcm_strategies_type ON rcm_strategies(strategy_type,status);
        CREATE TABLE IF NOT EXISTS rcm_strategy_reviews(
          id INTEGER PRIMARY KEY AUTOINCREMENT, review_no TEXT UNIQUE NOT NULL, strategy_id INTEGER NOT NULL REFERENCES rcm_strategies(id) ON DELETE CASCADE,
          old_status TEXT NOT NULL, outcome TEXT NOT NULL, notes TEXT NOT NULL, previous_review_due_date TEXT, next_review_due_date TEXT,
          reviewed_by INTEGER NOT NULL REFERENCES users(id), reviewed_at TEXT NOT NULL
        );
        CREATE INDEX IF NOT EXISTS idx_rcm_reviews_strategy ON rcm_strategy_reviews(strategy_id,reviewed_at);
        ...
        CREATE TABLE IF NOT EXISTS work_orders(
          ...
          work_type TEXT NOT NULL, failure_code TEXT DEFAULT '', asset_fmea_id INTEGER REFERENCES asset_fmea(id) ON DELETE SET NULL, requested_by INTEGER REFERENCES users(id), assigned_to INTEGER REFERENCES users(id), supervisor_id INTEGER REFERENCES users(id),
        ...
        CREATE TABLE IF NOT EXISTS retention_holds(
          id INTEGER PRIMARY KEY AUTOINCREMENT, hold_no TEXT UNIQUE NOT NULL, data_class TEXT NOT NULL, record_key TEXT NOT NULL DEFAULT '*',
          reason TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'Active', placed_by INTEGER NOT NULL REFERENCES users(id), placed_at TEXT NOT NULL,
          released_by INTEGER REFERENCES users(id), released_at TEXT, release_reason TEXT DEFAULT ''
        );
        CREATE INDEX IF NOT EXISTS idx_retention_holds_active ON retention_holds(data_class,status,record_key);
        CREATE TABLE IF NOT EXISTS retention_runs(
          id INTEGER PRIMARY KEY AUTOINCREMENT, run_no TEXT UNIQUE NOT NULL, mode TEXT NOT NULL, data_class TEXT, status TEXT NOT NULL,
          requested_by INTEGER NOT NULL REFERENCES users(id), started_at TEXT NOT NULL, finished_at TEXT, summary_json TEXT NOT NULL DEFAULT '{}',
          manifest_json TEXT NOT NULL DEFAULT '{}', prev_hash TEXT DEFAULT '', manifest_hash TEXT DEFAULT '', error_message TEXT DEFAULT ''
        );
        CREATE INDEX IF NOT EXISTS idx_retention_runs_time ON retention_runs(started_at,status);
        CREATE INDEX IF NOT EXISTS idx_retention_runs_hash ON retention_runs(manifest_hash);
        CREATE TABLE IF NOT EXISTS retention_run_items(
          id INTEGER PRIMARY KEY AUTOINCREMENT, run_id INTEGER NOT NULL REFERENCES retention_runs(id) ON DELETE CASCADE, policy_id INTEGER NOT NULL REFERENCES retention_policies(id),
          data_class TEXT NOT NULL, cutoff TEXT NOT NULL, eligible_count INTEGER NOT NULL DEFAULT 0, held_count INTEGER NOT NULL DEFAULT 0,
          protected_count INTEGER NOT NULL DEFAULT 0, blocked_count INTEGER NOT NULL DEFAULT 0, purged_count INTEGER NOT NULL DEFAULT 0, details_json TEXT NOT NULL DEFAULT '{}'
        );
        ...
        CREATE TABLE IF NOT EXISTS telemetry_ingest_batches(
          ...
          suppressed_count INTEGER NOT NULL DEFAULT 0, cbm_events_opened INTEGER NOT NULL DEFAULT 0, cbm_events_resolved INTEGER NOT NULL DEFAULT 0,
          cbm_work_orders_created INTEGER NOT NULL DEFAULT 0, ingested_by INTEGER REFERENCES users(id), started_at TEXT NOT NULL, completed_at TEXT
        );
        ...
        CREATE TABLE IF NOT EXISTS cbm_rules(
          id INTEGER PRIMARY KEY AUTOINCREMENT, rule_no TEXT UNIQUE NOT NULL, name TEXT NOT NULL,
          channel_id INTEGER NOT NULL REFERENCES telemetry_channels(id) ON DELETE CASCADE,
          operator TEXT NOT NULL, threshold_low REAL, threshold_high REAL, consecutive_readings INTEGER NOT NULL DEFAULT 1,
          cooldown_minutes INTEGER NOT NULL DEFAULT 60, severity TEXT NOT NULL DEFAULT 'Warning', action_type TEXT NOT NULL DEFAULT 'Recommendation',
          work_priority TEXT NOT NULL DEFAULT 'High', instructions TEXT DEFAULT '', asset_fmea_id INTEGER REFERENCES asset_fmea(id) ON DELETE SET NULL, active INTEGER NOT NULL DEFAULT 1,
          created_by INTEGER NOT NULL REFERENCES users(id), created_at TEXT NOT NULL, updated_by INTEGER REFERENCES users(id), updated_at TEXT NOT NULL
        );
        CREATE INDEX IF NOT EXISTS idx_cbm_rules_channel_active ON cbm_rules(channel_id,active);
        CREATE TABLE IF NOT EXISTS cbm_events(
          id INTEGER PRIMARY KEY AUTOINCREMENT, event_no TEXT UNIQUE NOT NULL, rule_id INTEGER NOT NULL REFERENCES cbm_rules(id) ON DELETE CASCADE,
          channel_id INTEGER NOT NULL REFERENCES telemetry_channels(id) ON DELETE CASCADE, asset_id INTEGER NOT NULL REFERENCES assets(id) ON DELETE CASCADE,
          reading_id INTEGER REFERENCES telemetry_readings(id) ON DELETE SET NULL, severity TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'Open',
          trigger_value REAL NOT NULL, message TEXT NOT NULL, work_order_id INTEGER REFERENCES work_orders(id), asset_fmea_id INTEGER REFERENCES asset_fmea(id) ON DELETE SET NULL, opened_at TEXT NOT NULL, last_seen_at TEXT NOT NULL,
          occurrence_count INTEGER NOT NULL DEFAULT 1, acknowledged_by INTEGER REFERENCES users(id), acknowledged_at TEXT, resolved_at TEXT, resolution_reason TEXT DEFAULT ''
        );
        CREATE INDEX IF NOT EXISTS idx_cbm_events_status ON cbm_events(status,severity,opened_at);
        CREATE INDEX IF NOT EXISTS idx_cbm_events_rule ON cbm_events(rule_id,status,last_seen_at);
        CREATE INDEX IF NOT EXISTS idx_cbm_events_asset ON cbm_events(asset_id,status,last_seen_at);
        CREATE TABLE IF NOT EXISTS cbm_rule_state(
          rule_id INTEGER PRIMARY KEY REFERENCES cbm_rules(id) ON DELETE CASCADE, consecutive_hits INTEGER NOT NULL DEFAULT 0,
          last_value REAL, last_quality TEXT DEFAULT '', last_evaluated_at TEXT, last_triggered_at TEXT, active_event_id INTEGER REFERENCES cbm_events(id) ON DELETE SET NULL
        );
        ...
        conn.execute('CREATE INDEX IF NOT EXISTS idx_cbm_events_fmea ON cbm_events(asset_fmea_id,status,last_seen_at)')
        ...
        # v4.6 fine-grained permission catalog. Existing coarse permissions remain for compatibility.
        permission_catalog=[
          ('admin.users.manage','Manage users','Administration','Critical','Create, activate and deactivate user accounts.'),
          ('admin.permissions.manage','Manage permissions','Administration','Critical','Change role grants and user-specific permission overrides.'),
          ('assets.write','Modify assets','Assets','Elevated','Create or update asset master data and operational readings.'),
          ('work.write','Plan work','Work Management','Elevated','Create and edit work orders and maintenance plans.'),
          ('work.transition','Execute work','Work Management','Elevated','Transition work, record labor/tasks and perform field execution actions.'),
          ('inventory.transact','Transact inventory','Inventory','Elevated','Reserve, issue and transact maintenance inventory.'),
          ('procurement.write','Manage procurement','Procurement','Elevated','Approve requisitions, quotations and purchase orders.'),
          ('hse.write','Manage HSE','HSE','Elevated','Create and update HSE incidents.'),
          ('projects.write','Manage projects','Projects','Elevated','Create projects and update project tasks.'),
          ('documents.upload','Upload documents','Documents','Elevated','Upload governed documents and attachments.'),
          ('telemetry.configure','Configure telemetry','Utilities Operations','Elevated','Create or change telemetry channels and topology configuration.'),
          ('cbm.rules.manage','Manage condition-based maintenance rules','Utilities Operations','Elevated','Create, edit, activate and deactivate deterministic condition-based maintenance rules.'),
          ('reliability.fmea.manage','Manage FMEA and failure modes','Maintenance & Reliability','Elevated','Create failure-mode hierarchies, asset FMEA records and governed FMEA reviews.'),
          ('reliability.rcm.manage','Manage RCM strategies','Maintenance & Reliability','Elevated','Create, revise, activate and review reliability-centered maintenance strategies.'),
          ('reliability.rcm.approve','Approve RCM strategies','Maintenance & Reliability','Critical','Approve or reject submitted RCM strategies through governed electronic approval.'),
          ('alarms.operate','Operate alarms','Utilities Operations','Elevated','Acknowledge alarms, request shelving and operate suppressions.'),
          ('approvals.decide','Decide assigned approvals','Governance','Critical','Approve or reject approval requests when assignment rules also permit it.'),
          ('governance.retention.execute','Execute retention','Governance','Critical','Run destructive governed data-retention execution.'),
          ('governance.legal_hold.manage','Manage legal holds','Governance','Critical','Place and release legal holds.'),
          ('automation.run','Run automation','Automation','Elevated','Run maintenance automation and controlled operational jobs.'),
          ('integration.keys.manage','Manage integration keys','Integrations','Critical','Create, rotate or revoke machine-to-machine integration keys.')
        ]
        newly_added_permissions=[]
        for code,name,category,risk,description in permission_catalog:
            existed=conn.execute('SELECT id FROM permissions WHERE code=?',(code,)).fetchone()
            if not existed:
                conn.execute('INSERT INTO permissions(code,name,category,risk_level,description) VALUES(?,?,?,?,?)',(code,name,category,risk,description))
                newly_added_permissions.append(code)
            else:
                conn.execute('UPDATE permissions SET name=?,category=?,risk_level=?,description=? WHERE code=?',(name,category,risk,description,code))
        role_ids={r['code']:r['id'] for r in conn.execute('SELECT * FROM roles')}
        perm_ids={p['code']:p['id'] for p in conn.execute('SELECT * FROM permissions')}
        defaults={
          'admin':[x[0] for x in permission_catalog],
          'asset_manager':['assets.write','work.write','documents.upload','telemetry.configure','cbm.rules.manage','reliability.fmea.manage','reliability.rcm.manage','alarms.operate'],
          'maintenance_manager':['assets.write','work.write','work.transition','inventory.transact','procurement.write','documents.upload','telemetry.configure','cbm.rules.manage','reliability.fmea.manage','reliability.rcm.manage','reliability.rcm.approve','alarms.operate','approvals.decide','automation.run'],
          'planner':['assets.write','work.write','work.transition','inventory.transact','documents.upload','telemetry.configure','cbm.rules.manage','reliability.fmea.manage','reliability.rcm.manage','alarms.operate','approvals.decide','automation.run'],
          'supervisor':['assets.write','work.write','work.transition','documents.upload','telemetry.configure','reliability.fmea.manage','reliability.rcm.manage','alarms.operate','approvals.decide'],
          'technician':['work.transition','inventory.transact','documents.upload','alarms.operate'],
          'storekeeper':['inventory.transact','documents.upload'],
          'procurement':['procurement.write','documents.upload','approvals.decide'],
          'hse':['hse.write','documents.upload'],
          'project_manager':['projects.write','documents.upload']
        }
        for role,codes in defaults.items():
            if role not in role_ids: continue
            for code in codes:
                if code in newly_added_permissions and code in perm_ids:
                    conn.execute('INSERT OR IGNORE INTO role_permissions(role_id,permission_id) VALUES(?,?)',(role_ids[role],perm_ids[code]))
        ...
        if conn.execute('SELECT COUNT(*) FROM technician_profiles').fetchone()[0] == 0:
            ...
            today=date.today();seed_week=today-timedelta(days=today.weekday());leave_start=seed_week+timedelta(days=14);leave_end=leave_start+timedelta(days=1)
            ...
            conn.execute('INSERT INTO technician_absences(user_id,start_date,end_date,absence_type,hours_per_day,status,notes,created_by,created_at) VALUES(?,?,?,?,?,?,?,?,?)',
              (users_map['tech2'],leave_start.isoformat(),leave_end.isoformat(),'Annual Leave',8,'Approved','Demo planned leave',users_map['supervisor'],now()))
