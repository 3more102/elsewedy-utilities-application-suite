...
def test_v44_signature_chain_detects_tampering_and_preserves_delegation_evidence():
    with TestClient(app) as client:
        ...
        assert manifest['version'] == '4.4.0' and manifest['schema_version'] == 14
        assert manifest['api_endpoints'] == len(all_routes) == 195
        assert manifest['api_routes_under_api'] == len(api_routes) == 194
        assert manifest['relational_tables'] == len(re.findall(r'CREATE TABLE IF NOT EXISTS\s+', db_src, re.I)) == 71
        assert manifest['explicit_indexes'] == len(re.findall(r'CREATE (?:UNIQUE )?INDEX IF NOT EXISTS\s+', db_src, re.I)) == 55
        assert manifest['automated_tests'] == 22
        assert 'version-4.4.0' in readme and 'schema-v14' in readme and 'regression_tests-22' in readme
        assert "euas-shell-v4.4.0" in sw
