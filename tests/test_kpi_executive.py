...
def test_saidi_saifi_caidi_formulas_match_customer_weighting():
    ...
    with db() as conn:
        cur = conn.execute(
            '''INSERT INTO sites(site_code,name,region,city,site_type,status,customer_count)
    ...
    assert rel['customer_count_total'] == 10000
...
def test_zero_data_behavior_is_well_defined():
    ids = _ids()
    with db() as conn:
        conn.execute('UPDATE sites SET customer_count=0')
...
def test_partial_window_overlap_only_counts_inside_period():
    ids = _ids()
    # A 7-day window ending today starts at (today-6d) 00:00. An outage that
    # straddles that boundary must contribute only its in-window tail.
    period_start = datetime.combine(
        date.today() - timedelta(days=6), datetime.min.time())
    start = period_start - timedelta(hours=3)
    end = period_start + timedelta(hours=3)
    _seed_outage(ids, asset_id=ids['pmp'], site_id=ids['iwp'],
                 start=start.isoformat(timespec='seconds'),
                 end=end.isoformat(timespec='seconds'), code_suffix='P1')

    f = ExecutiveFilters(period_days=7, site_id=ids['iwp'])
    with db() as conn:
        rel = compute_reliability(conn, f)
        rel_prev_scope = ExecutiveFilters(period_end=(datetime.now() - timedelta(days=9)).date().isoformat(), period_days=7, site_id=ids['iwp'])
    ...
    assert abs(rel['total_downtime_hours'] - 3.0) < 0.05  # exactly the in-window half
