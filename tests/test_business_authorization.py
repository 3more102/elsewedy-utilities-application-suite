...
def test_business_capability_revocation_is_immediate_for_existing_session():
    with TestClient(app) as client:
        ...
        try:
            with db() as conn:
                replace_role_permissions(conn, 'admin', [code for code in original if code != 'assets.health.recalculate'])
...
def test_business_capability_cannot_expand_historical_role_ceiling():
    with TestClient(app) as client:
        ...
        try:
            with db() as conn:
                replace_role_permissions(conn, 'executive', sorted(set(original) | {'assets.health.recalculate'}))
...
def test_procurement_approval_revocation_is_immediate_before_resource_lookup():
    ...
    with TestClient(app) as client:
        ...
        baseline = client.post(f'/api/procurement/requisitions/{missing_pr}/approve', headers=admin)
        assert baseline.status_code == 404, baseline.text
        try:
            with db() as conn:
                replace_role_permissions(conn, 'admin', [code for code in original if code != 'procurement.requisition.approve'])
            denied = client.post(f'/api/procurement/requisitions/{missing_pr}/approve', headers=admin)
            assert denied.status_code == 403, denied.text
        finally:
            with db() as conn:
                replace_role_permissions(conn, 'admin', original)
        restored = client.post(f'/api/procurement/requisitions/{missing_pr}/approve', headers=admin)
...
def test_procurement_approval_capability_cannot_promote_executive_role():
    ...
    with TestClient(app) as client:
        ...
        try:
            with db() as conn:
                replace_role_permissions(conn, 'executive', sorted(set(original) | {'procurement.requisition.approve'}))
            denied = client.post(f'/api/procurement/requisitions/{missing_pr}/approve', headers=executive)
            assert denied.status_code == 403, denied.text
        finally:
            with db() as conn:
                replace_role_permissions(conn, 'executive', original)
...
def test_project_task_revocation_is_immediate_before_project_lookup():
    ...
    with TestClient(app) as client:
        ...
        baseline = client.post(f'/api/projects/{missing_project}/tasks', headers=admin, json=payload)
        assert baseline.status_code == 404, baseline.text
        try:
            with db() as conn:
                replace_role_permissions(conn, 'admin', [code for code in original if code != 'projects.tasks.create'])
            denied = client.post(f'/api/projects/{missing_project}/tasks', headers=admin, json=payload)
            assert denied.status_code == 403, denied.text
        finally:
            with db() as conn:
                replace_role_permissions(conn, 'admin', original)
        restored = client.post(f'/api/projects/{missing_project}/tasks', headers=admin, json=payload)
        assert restored.status_code == 404, restored.text
...
def test_project_task_capability_cannot_promote_executive_role():
    ...
    with TestClient(app) as client:
        ...
        try:
            with db() as conn:
                replace_role_permissions(conn, 'executive', sorted(set(original) | {'projects.tasks.create'}))
            denied = client.post(f'/api/projects/{missing_project}/tasks', headers=executive, json=payload)
            assert denied.status_code == 403, denied.text
        finally:
            with db() as conn:
                replace_role_permissions(conn, 'executive', original)
