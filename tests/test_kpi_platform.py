...
regression bounds, region propagation, customer_count administration, and
...
def test_customer_count_admin_workflow():
    _ensure_db()
    with db() as conn:
    ...
    with TestClient(app) as client:
        ...
        assert client.patch(f'/api/sites/{site_id}/customer-count', headers=tech,
                            json={'customer_count': 500}).status_code == 403
        assert client.patch(f'/api/sites/{site_id}/customer-count', headers=planner,
                            json={'customer_count': 500}).status_code == 403
        assert client.patch(f'/api/sites/{site_id}/customer-count',
                            json={'customer_count': 500}).status_code == 401

        # Validation: negative values are rejected cleanly.
        bad = client.patch(f'/api/sites/{site_id}/customer-count', headers=admin,
                           json={'customer_count': -5})
        assert bad.status_code == 422

        # Configured path updates value, is visible, and unlocks SAIDI basis.
        ok = client.patch(f'/api/sites/{site_id}/customer-count', headers=admin,
                          json={'customer_count': 25000})
        assert ok.status_code == 200, ok.text
        assert ok.json()['customer_count'] == 25000
        ref = client.get('/api/reference', headers=admin).json()
        site = next(s for s in ref['sites'] if s['id'] == site_id)
        assert site['customer_count'] == 25000
        ...
        clear = client.patch(f'/api/sites/{site_id}/customer-count', headers=admin,
                             json={'customer_count': 0})
        ...
        # Unknown site -> 404.
        assert client.patch('/api/sites/999999999/customer-count', headers=admin,
                            json={'customer_count': 1}).status_code == 404
