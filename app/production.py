...
an immediate socket peer covered by ``EUAS_TRUSTED_PROXY_CIDRS`` may supply a
single ``X-Forwarded-Proto: http|https`` value. This keeps forwarded-scheme trust
aligned with the spoof-resistant client-identity boundary and preserves the raw
socket peer for application-level X-Forwarded-For processing.
"""
