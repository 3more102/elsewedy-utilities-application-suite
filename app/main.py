...
from core.configuration import APP_NAME, APP_VERSION, STATIC_DIR, UPLOAD_DIR, SESSION_HOURS, MAX_UPLOAD_BYTES, MAX_UPLOAD_MB, ALLOWED_DOC_SUFFIXES, DB_BACKEND, DB_PATH, SCHEMA_VERSION, AUTOMATION_INTERVAL_MINUTES, EVENT_WEBHOOK_URL, EVENT_WEBHOOK_SECRET, OUTBOX_MAX_ATTEMPTS
...
from apps.observability import health_snapshot, job_metric_lines, job_metrics_snapshot, readiness_snapshot, request_metrics_snapshot
...
def metrics(user=Depends(require_roles('admin','maintenance_manager','executive'))):
    ...
    with db() as conn:
        ...
        job_metrics=job_metrics_snapshot(conn)
    ...
    lines.extend(job_metric_lines(job_metrics))
