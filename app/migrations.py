...
def _secure_seed_hasher(hash_password, secret: str):
    ...
    def secure_hash(password: str) -> str:
        if password in defaults:
            # The legacy bootstrap supplies only the plaintext password, not the
            # username. Replace every packaged demo password before the seed
            # transaction is committed; the administrator account is then set to
            # the operator-provided secret in the post-bootstrap transaction.
