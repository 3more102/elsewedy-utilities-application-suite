...
- role and fine-grained permission enforcement
- request validation
...
EUAS v4.6 evaluates permissions in the API layer before governed mutations. Active per-user Allow/Deny overrides take precedence over the current role grant; otherwise authorization denies by default. Domain workflow rules remain a second gate after permission evaluation. The access-control Administration UI reads the same catalog/effective model but never substitutes for server checks.

### Persistence layer

EUAS v4.9.0 uses a database adapter with two modes: SQLite for the zero-configuration reference deployment and PostgreSQL when `EUAS_DATABASE_URL` is supplied. Both modes share the same service/API boundary, relational business identifiers and workflow model. SQLite uses foreign keys/WAL/indexes; the PostgreSQL adapter translates bind syntax and reference DDL while preserving transactions and generated identifiers.

## 3. Module Boundaries

- **Foundation:** users, roles, permission catalog, role grants, user permission overrides, sessions, sites, locations, vendors.
...
`report_snapshots` stores serialized asset-dossier point-in-time data plus a SHA-256 content hash. No update/delete report-snapshot API is exposed. A verification endpoint recomputes the hash to detect mutation. `backup_records` stores evidence for administrator-generated SQLite backup packages, including application version, byte size and returned SHA-256. `retention_policies` stores policy intent while `retention_holds`, `retention_runs` and `retention_run_items` implement governed execution. Destructive runs require admin credential re-authentication and explicit confirmation, active legal holds override eligibility, protected classes are never purged, and canonical run manifests form a separate SHA-256 evidence chain. Portable evidence packages support external immutable archiving without claiming local WORM semantics.
...
## Reliability and FMEA services (v4.8)

`failure_modes` provides a cycle-safe hierarchy of reusable failure modes. `asset_fmea` binds a failure mode to an asset and stores effects, causes, controls, recommended action, review ownership and server-calculated Severity × Occurrence × Detectability RPN. `fmea_reviews` preserves formal rating changes as review evidence.

FMEA linkage is intentionally reused by the existing work-management and CBM pipelines rather than introducing a parallel execution engine. FMEA-generated work enters the normal Submitted/Approval workflow, while a linked CBM event carries the same `asset_fmea_id` into generated work for end-to-end reliability traceability.

## Reliability-Centered Maintenance strategy services (v4.9)

`rcm_strategies` adds a governed maintenance-policy layer above asset FMEA. Each strategy binds exactly one FMEA record to a consequence classification and one of five explicit strategy types. The service reuses existing CBM rules and preventive-maintenance plans by validated identifiers rather than creating duplicate condition-monitoring or scheduling engines.

Strategy submission routes through the existing Approval Center. Approval therefore inherits credential re-authentication, exact signer intent, four-eyes protection and hash-chained approval-signature evidence. A separate `reliability.rcm.approve` permission is required in addition to normal approval authority. Approved strategies are explicitly activated; formal reviews can Continue, return the strategy to Draft for Revise/reapproval, or Retire it.

RCM review dates are derived deterministically from the linked FMEA risk band. This is transparent reference logic for workflow/testing and not a claim of SAE JA1011/JA1012, IEC/ISO, OEM or regulated safety-case conformance. See `RCM_STRATEGIES.md`.
