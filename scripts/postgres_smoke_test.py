...
def main() -> int:
    ...
    try:
        ...
        status, _, work = request(
            '/api/work-orders', method='POST', token=token,
            data={
        ...
        status, _, channel = request(
            '/api/telemetry/channels', method='POST', token=token,
            data={
        ...
        status, _, ingestion = request(
            '/api/telemetry/ingest', method='POST', token=token,
        ...
        status, _, readings = request(f'/api/telemetry/readings?channel_id={channel_id}&hours=24', token=token)
        assert status == 200 and readings and readings[0]['channel_code'] == channel_code

        status, _, alarm_work = request(f'/api/alarms/{alarm_id}/work-order', method='POST', token=token, data={})
