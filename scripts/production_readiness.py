...
def evaluate_configuration(
    ...
    if strict_production:
        checks.append(Check("environment", "PASS" if environment == "production" else "FAIL", f"EUAS_ENV={environment!r}; strict mode requires 'production'."))
    else:
    ...
    checks.append(Check("automation_scheduler", "PASS" if automation_minutes > 0 else "WARN", f"Automation interval is {automation_minutes} minute(s)." if automation_minutes > 0 else "In-process scheduler disabled; an external scheduler/worker must trigger automation."))
    checks.append(Check("session_lifetime", "PASS" if 1 <= session_hours <= 24 else "WARN", f"Session lifetime is {session_hours} hour(s)."))
    checks.append(Check("upload_limit", "PASS" if 1 <= upload_mb <= 100 else "WARN", f"Maximum upload size is {upload_mb} MiB."))
    return checks
...
def run_database_checks() -> list[Check]:
    ...
    from app.migrations import find_insecure_demo_users, initialize_database, migration_status
    ...
    critical_tables = {
        "users", "auth_sessions", "auth_login_throttle", "assets", "work_orders",
        "inventory_items", "audit_logs", "job_runs", "event_outbox",
        "telemetry_channels", "telemetry_readings", "operational_alarms",
    }
    try:
        with db() as conn:
            ...
            checks.append(Check("critical_tables", "FAIL" if missing else "PASS", f"Missing: {', '.join(missing)}" if missing else f"All {len(critical_tables)} critical tables present."))
            ...
            migration = migration_status(conn, backend=DB_BACKEND, target_version=SCHEMA_VERSION)
            checks.append(Check(
                "schema_migrations",
                "PASS" if migration["ready"] else "FAIL",
                f"current={migration['current_version']}, target={migration['target_version']}, pending={migration['pending_versions']}, invalid={migration['invalid_versions']}, future={migration['future_versions']}.",
            ))
            ...
            if report["valid"]:
                checks.append(Check("audit_chain_integrity", "PASS", f"checked={report['checked']}, head_hash={report['head_hash'] or '-'}."))
            else:
                checks.append(Check("audit_chain_integrity", "FAIL", f"Chain invalid at record {report['first_invalid_id']} (checked={report['checked']}, last_good_head={report['head_hash'] or '-'})."))
...
def main() -> int:
    ...
    checks = evaluate_configuration(os.environ, require_postgres=args.require_postgres, strict_production=args.strict_production)
    if args.check_db:
