...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="euas-smoke-") as td:
        ...
        try:
            ...
            status, _, permissions = request("/api/auth/me/permissions", token=token)
            assert status == 200 and permissions["role"] == "admin"
            assert "admin.permissions.manage" in permissions["allowed"]
            assert "work.write" in permissions["allowed"]

            status, _, access_control = request("/api/admin/access-control", token=token)
            assert status == 200 and access_control["confirmation_phrase"] == "UPDATE ACCESS"
            assert any(p["code"] == "admin.permissions.manage" for p in access_control["permissions"])
            ...
            status, _, retention_preview = request("/api/governance/retention/preview", token=token)
            assert status == 200 and any(x["data_class"] == "Notifications" and x["execution_supported"] for x in retention_preview)

            status, _, retention_chain = request("/api/governance/retention/verify", token=token)
            assert status == 200 and retention_chain["valid"] is True
            ...
            assert status == 200 and field_sync["schema_version"] >= 19 and isinstance(field_sync["work_orders"], list)
            ...
            status, _, cbm_rules = request("/api/cbm/rules", token=token)
            assert status == 200 and isinstance(cbm_rules, list)

            status, _, cbm_events = request("/api/cbm/events?limit=5", token=token)
            assert status == 200 and isinstance(cbm_events, list)

            status, _, fmea_summary = request("/api/reliability/summary", token=token)
            assert status == 200 and "risk_bands" in fmea_summary and "active_records" in fmea_summary

            status, _, failure_modes = request("/api/reliability/failure-modes", token=token)
            assert status == 200 and isinstance(failure_modes, list)

            status, _, fmea_records = request("/api/reliability/fmea?limit=5", token=token)
            assert status == 200 and isinstance(fmea_records, list)
            assert "reliability.fmea.manage" in permissions["allowed"]

            status, _, rcm_summary = request("/api/reliability/rcm/summary", token=token)
            assert status == 200 and "strategy_coverage_pct" in rcm_summary and "critical_uncovered" in rcm_summary

            status, _, rcm_strategies = request("/api/reliability/rcm", token=token)
            assert status == 200 and isinstance(rcm_strategies, list)
            assert "reliability.rcm.manage" in permissions["allowed"]
            assert "reliability.rcm.approve" in permissions["allowed"]
