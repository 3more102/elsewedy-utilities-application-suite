...
v4.7 evaluates governed rules directly in the telemetry ingestion transaction after a Good-quality reading is persisted. Rules support scalar and range comparisons, consecutive-reading confirmation, cooldown, severity and either recommendation-only or governed work-order action. Generated work orders are submitted into the existing approval workflow rather than bypassing maintenance governance. Bad and Uncertain readings are retained for data-quality evidence but do not advance CBM state.

CBM events retain the source rule, channel, asset, source reading, trigger value, occurrences, lifecycle and generated work-order link. Automatic resolution occurs only after a later Good-quality reading no longer matches the rule.
